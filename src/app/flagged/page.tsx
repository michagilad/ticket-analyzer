...
function EkoEmbed({ instanceId }: { instanceId: string }) {
  ...
  return (
    <div 
      ...
      className="mx-auto bg-white rounded-lg overflow-hidden border border-gray-300"
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <Flag className="w-5 h-5 text-[#6B52FF]" />
                  <h1 className="text-xl font-semibold text-gray-900">Flagged Experiences</h1>
                  {isAdminMode && (
                    <span className="text-xs px-2 py-1 rounded-full bg-[#6B52FF]/10 text-[#6B52FF] border border-[#6B52FF]/20 font-medium">
                      ADMIN MODE
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-50 border border-gray-200">
                  ...
                  <span className="text-sm text-gray-900 font-medium">{format(parseISO(selectedDate), 'MMM d, yyyy')}</span>
                </div>
                <button
                  onClick={() => setShowCalendar(!showCalendar)}
                  className="px-3 py-1.5 rounded-lg bg-[#6B52FF]/10 border border-[#6B52FF]/20 text-[#6B52FF] text-sm font-medium hover:bg-[#6B52FF]/20 transition-colors"
                >
                ...
                {isAdminMode && totalCount > 0 && (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-red-50 border border-red-200 text-red-600 text-sm font-medium hover:bg-red-100 transition-colors"
          ...
          <div className="mb-6 bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-700 min-w-32 text-center">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              {isEditing && editedExperience ? (
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs text-gray-400 uppercase tracking-wide block mb-2">Instance ID</label>
                    <input
                      type="text"
                      value={editedExperience.instanceId}
                      onChange={(e) => setEditedExperience({ ...editedExperience, instanceId: e.target.value })}
                      className="w-full px-3 py-2 bg-white border border-gray-400 rounded-lg text-gray-900 focus:outline-none focus:border-violet-500"
                    />
                  </div>
                  <div>
                    <label className="text-xs text-gray-400 uppercase tracking-wide block mb-2">Ticket Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="text-xs text-gray-400 uppercase tracking-wide block mb-2">Ticket Description</label>
                    <textarea
                  ...
                  <div>
                    <label className="text-xs text-gray-400 uppercase tracking-wide block mb-2">Backstage Link</label>
                    <input
                      type="text"
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    <button
                      onClick={saveEdits}
                      className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 text-gray-900 hover:bg-emerald-500 transition-colors"
                    >
                ...
                <div className="space-y-5">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`
                          ...
                          ${currentExperience.ticketStatus.toLowerCase() === 'in progress' 
                            ...
                            : 'bg-slate-500/20 text-gray-500 border border-slate-500/30'
                          }
                        `}>
                  ...
                  <div className="space-y-3">
                    <div>
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ticket</span>
                      <p className="text-gray-900 mt-1 font-medium">{currentExperience.ticketName}</p>
                    </div>
                    {currentExperience.ticketDescription && (
                      <div>
                        ...
                        <p className="text-gray-700 mt-1 text-sm leading-relaxed">{currentExperience.ticketDescription}</p>
                      </div>
                    )}
                    <div>
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Instance ID</span>
                      <p className="text-gray-700 mt-1 font-mono text-sm bg-white/50 px-3 py-1.5 rounded-lg inline-block">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[#6B52FF] hover:bg-[#6B52FF] text-gray-900 font-medium transition-all shadow-lg shadow-[#6B52FF]/20 text-sm"
                    >
                    ...
                    {currentExperience.backstageLink && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-900 font-medium transition-colors text-sm"
                      >
                        <ExternalLink className="w-4 h-4" />
                        Open in Backstage
                      </a>
            ...
            <div className="flex items-center justify-between gap-3 bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              ...
              <div className="text-center">
                <div className="text-gray-500 text-xs font-medium uppercase tracking-wider mb-0.5">Experience</div>
                <div className="text-gray-900 font-bold text-lg">
                  {currentIndex + 1} <span className="text-gray-400 font-normal">of</span> {totalCount}
                </div>
              </div>
